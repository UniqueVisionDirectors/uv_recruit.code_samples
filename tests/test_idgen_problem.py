...
このファイルは、`app/idgen/problem.py` の `ProblemIssuer.issue` を実装する演習の
自動採点テストである。既定の pytest ゲートからは `exercise` / `exercise2`
マーカーで除外され（出荷時は未解答＝これらは赤）、演習を解く人は次のように
明示的に実行して red→green を体験する。

  - ステージ1（基本要件）:  `pytest -m exercise`
  - ステージ2（worker-id）:  `pytest -m exercise2`

足場の `ProblemIssuer` は最初から `worker_id`・`now_ms`・`rng` を受け取り、
`self._worker_id` を保持する。ステージ1（ch03/ch04）では worker_id には触れず、
`now_ms`/`rng` だけで `issue()` を実装する。ステージ2（ch07）では、すでに用意済みの
`self._worker_id` を `issue()` の中で使うようにすると、TC5/TC6 が緑になる。
...
■ 1. ID要件の整理（検証可能な定義）
  ...
  ── ステージ2（並列 / すでにある self._worker_id を使う）
...
■ 2. デシジョンテーブル（2つのIDが衝突するか）
  ...
  -------+-----------+--------+------
    ...
    N    |    -      |   -    | しない（発行点の差＝R3＝TC3）
  → 同一ms・異worker が「衝突しない」ことが ステージ2 の肝（TC5）。
...
■ 4. テストケース（要件・技法 → TC）
  [exercise]  TC1 R1+R2 : 多数発行が全て ^[0-9A-Za-z]{10}$       （EP: 文字種/長さ）
  [exercise]  TC2 R5    : 同一ms・単一発行器で 4096件 全 distinct（BVA: 容量上限）
  [exercise]  TC3 R3    : 複数の発行点で発行順 == ソート順        （単調性）
...
■ 5. 偽陽性・偽陰性を出さない根拠
  ...
  偽陽性なし（誤実装で該当 TC が赤）:
    TC1=長さ/文字種, TC3=発行順を保てない, TC4=開始固定(連番),
    TC5=worker 無視(鳩の巣で必ず衝突), TC6=worker ビット位置/逆順ミス を各々検出。

■ 6. 実装上の注意
  - クロックは `_spin_safe_clock` を使う：通常は固定 ms を返すが、実装が容量超過時の
  ...
  - TC1–TC6 は `issue()` を呼ぶため未実装スタブでは赤。V1 は `__init__`（足場）の
    worker_id 検証のみを見るため、未実装でも緑（既定ゲートに置く）。
"""
...
# worker_id は渡すが、ステージ1の実装は使わなくてよい（0 を渡す）。
...
def test_tc3_sortable_by_issue_order() -> None:
    """TC3 (R3): 発行点（時刻）を進めて発行 → 発行順 == 文字列ソート順。"""
...
# ── ステージ2（並列の一意性 R6/R7）: `pytest -m exercise2` ────────────────────
# すでにある self._worker_id を issue() で使うと緑になる。
