...
- 前章の「偶然の重なり（誕生日問題）」が起きる余地そのものが、**台ごとに分かれて消える** のです。
...
ここでは「枠（4096）より多く（8192）作る」ので、衝突が**確実に**起きます。これが鳩の巣原理の出番です。
...
- 複数台になると、互いを知らない台どうしが **同じ番号空間** を取り合い、**確率的に衝突** した（誕生日問題）。
- **worker-id** で台ごとに番号空間を分けることで、衝突の余地そのものをなくせた。

IDのビット構成の詳しい内訳は、[付録](/08-appendix) を参照してください。
